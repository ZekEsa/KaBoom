...
public class FingerprintLib {
    ...
    public static int[] getKeyFrequenciesFor(Complex[] results) {
        ...
        int[] output = new int[ ranges.length-1];
        ...
        for (int i = 0; i < ranges.length - 1; i++) {
            ...
            output[i] = loudestFreq;
        }
        return output;
    }

    private static int getLoudestIndexBetween(int low, int high, Complex[] results) {
        int highestFreq = low;
        double maxVol = results[highestFreq].abs();

        for (int i = low; i < high; i++) {
            if(results[i].abs() > maxVol){
                highestFreq = i;
                maxVol = (int)results[i].abs();
            }
        }
